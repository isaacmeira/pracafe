...
- **Dev server caindo com EBUSY em `C:\DumpStack.log.tmp`.** Causa: a detecção automática de fontes do Tailwind v4 subia até a raiz do disco (projeto em pasta com espaço, `mais todos`, sem `.git` acima). O `watch.ignored` do Vite não resolveu. Regra: `@import 'tailwindcss' source('../');` no CSS para restringir a varredura a `src/`.
